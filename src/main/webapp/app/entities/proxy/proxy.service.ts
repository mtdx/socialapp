...
export class ProxyService {
    ...
    create(proxy: Proxy): Observable<Proxy> {
        const copy = this.convert(proxy);
        return this.http.post(this.resourceUrl, copy).map((res: Response) => {
            return res.json();
        });
    }

    update(proxy: Proxy): Observable<Proxy> {
        const copy = this.convert(proxy);
    ...
    find(id: number): Observable<Proxy> {
    ...
    private convert(proxy: Proxy): Proxy {
        const copy: Proxy = Object.assign({}, proxy);
        return copy;
    }
}
