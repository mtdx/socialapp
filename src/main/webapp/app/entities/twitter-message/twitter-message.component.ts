...
@Component({
    selector: 'jhi-twitter-message',
    templateUrl: './twitter-message.component.html'
})
export class TwitterMessageComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.twitterMessageService.search({
    ...
    trackId(index: number, item: TwitterMessage) {
        return item.id;
    }
    registerChangeInTwitterMessages() {
        this.eventSubscriber = this.eventManager.subscribe('twitterMessageListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.twitterMessages.push(data[i]);
        }
    }
